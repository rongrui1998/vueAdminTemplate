...
这份文档用于统一说明 `front/V2` 当前继承的 Base 能力基线，以及它作为 Standard 工作目录的起点状态：
...
1. 先看本文件，建立整体认知
2. 再看 `00-base-completion-summary.md`，确认当前继承进来的 Base 范围
3. 再看 `10-standard-roadmap.md`，了解 V2 的目标能力和阶段划分
4. 如果想确认原始 Base 是否已达到定版交付状态，再看 `09-base-release-notes.md`
5. 如果要接真实后端，再看 `08-backend-integration-contract.md`
6. 如果要按 Standard 交付或演示，再看 `11-standard-usage-guide.md` 和 `12-standard-demo-checklist.md`
7. 按需深入 `02 ~ 05` 这几份分章节文档

## 2. Base 版本是什么

`front/V2` 当前不是从零起步的新工程，而是一份 **继承了 V3 Base 的 Standard 开发工作区**。

它已经具备 Base 所需的主链路和工程护栏，当前需要做的是在这套基线上继续补齐 Standard 功能，而不是重复搭建登录、菜单、权限和基础布局。

当前这份基线仍然保留 Base 的 4 个核心价值：
...
### 11.6 导入导出示例已支持 mock/API 闭环

V2 Standard 已在「业务示例 / 导入导出示例」下提供 CSV 文件选择、后端预览校验、确认导入和导出下载示例。

当前示例不引入 Excel 解析库，也不做异步任务队列，主要用于说明标准后台里导入导出的同步闭环。真实项目可以按需替换为：
...
V2 Standard 已补充几个偏模板交付的收口能力：

- 「业务示例 / 标准业务模板」：组合查询、表格、弹窗、抽屉、状态切换和行操作布局，适合复制成真实业务页。
- 右侧固定设置抽屉：集中演示主题、布局密度、标签栏和页面缓存等本地偏好。
- Dashboard：增加快捷入口、系统状态、版本定位和数据模式信息。
- 错误页：`404` 和动态路由组件缺失页已增强引导文案和视觉层级。
